...
export default function ExpenseDetails({ projectId, setExpenseData }) {
              ...
              if (project.cid) {
                return fetch(`/api/customers?id=${project.cid}`)
                  ...
                  .then(customerData => {
                    ...
                    setCname(
                      prev => prev || customerData.cname || 'Unknown Customer'
                    );
                  })
  ...
  const totalExtraExpense = extraExpenses.reduce(
    (sum, exp) => sum + (exp.quantity * exp.unitPrice || 0),
    0
  );
            ...
            {extraExpenses.length > 0 && (
              <div className="mt-6 space-y-4">
                ...
                {extraExpenses.map((expense, index) => (
                  <div
                    key={index}
                    className="flex flex-col md:flex-row gap-3 items-start md:items-center bg-black/20 p-4 rounded-lg border border-white/10"
                  >
                    <input
                      type="text"
                    ...
                    <input
                      ...
                      onChange={e => {
                        const newExtraExpenses = [...extraExpenses];
                        newExtraExpenses[index].quantity = Number(
                          e.target.value
                        );
                        setExtraExpenses(newExtraExpenses);
                      }}
                    ...
                    <input
                      ...
                      onChange={e => {
                        const newExtraExpenses = [...extraExpenses];
                        newExtraExpenses[index].unitPrice = Number(
                          e.target.value
                        );
                        setExtraExpenses(newExtraExpenses);
                      }}
                    ...
                    <button
                      onClick={() =>
                        setExtraExpenses(
                          extraExpenses.filter((_, i) => i !== index)
                        )
                      }
