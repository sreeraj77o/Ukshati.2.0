...
export default function generatePurchaseOrderPDF(poData) {
  ...
  doc.text(
    ...
    15,
    70 + vendorAddressLines.length * 5
  );
    ...
    didParseCell: data => {
      ...
      if (data.column.index === 0 || data.column.index === 1) {
        ...
        if (data.cell.text[0] && data.cell.text[0].length > maxLength) {
          data.cell.text[0] =
            data.cell.text[0].substring(0, maxLength - 3) + '...';
        }
      }
    },
    didDrawPage: data => {
      ...
      if (data.table.width > 180) {
        console.warn(
          `Table width (${data.table.width}mm) exceeds safe page width (180mm)`
        );
      }
    },
  });
  ...
  doc.setFont('helvetica', 'normal');
  doc.text(`${(poData.subtotal ?? 0).toFixed(2)}`, 190, finalY, {
    align: 'right',
  });
  doc.text(`${(poData.tax_amount ?? 0).toFixed(2)}`, 190, finalY + 5, {
    align: 'right',
  });
  doc.setFont('helvetica', 'bold');
  doc.text(`${(poData.total_amount ?? 0).toFixed(2)}`, 190, finalY + 10, {
    align: 'right',
  });
