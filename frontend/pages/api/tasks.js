  ...
  switch (req.method) {
    ...
    case 'PUT':
      return handleDBOperation(async db => {
        ...
        updatedProject.end_date = updatedProject.end_date
        ...
        try {
          ...
          if (cascade === 'true') {
            if (stockTransactions[0].count > 0) {
              await db.query(
                'DELETE FROM stock_transactions WHERE project_id = ?',
                [pid]
              );
            }

            // Delete inventory_spent records if they exist
            await db.query('DELETE FROM inventory_spent WHERE used_for = ?', [
              pid,
            ]);
          }
          ...
          if (quotes && quotes[0].count > 0) {
            await db.query('DELETE FROM quotesdata WHERE project_id = ?', [
              pid,
            ]);
          }
