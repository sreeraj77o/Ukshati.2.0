...
export default function Dashboard() {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const [stocksRes, categoriesRes, spentRes, stockCount] =
        await Promise.all([
          fetch('/api/stocks'),
      ...
      const totalStockValue = stocksData.reduce(
        (sum, item) => sum + item.quantity * item.price_pu,
        0
      );
      ...
      {isLoading ? (
        ...
        <>
          <div className="flex items-center justify-between">
            ...
            <div
              className={`p-3 rounded-full ${trend?.color || 'bg-gray-700'}`}
            >
          ...
          {trend && (
            <div className="mt-4 flex items-center space-x-2">
              <trend.icon
                className={`text-sm ${trend.value > 0 ? 'text-green-400' : 'text-red-400'}`}
              />
              <span
                className={`text-sm ${trend.value > 0 ? 'text-green-400' : 'text-red-400'}`}
              >
                ...
                ? // Skeleton for Quick Actions
                  Array(4)
                    ...
                    .map((_, i) => (
                      <div
                        key={i}
                        className="bg-gray-700 p-3 md:p-4 rounded-lg flex items-center gap-3 animate-pulse"
                      >
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <div
                          className={`p-2 rounded-lg ${item.quantity < 10 ? 'bg-red-500' : 'bg-blue-500'}`}
                        >
