...
export default function InventorySpent() {
    ...
    const fetchData = async () => {
      try {
        ...
        const spentWithDetails = spentData.map(spent => ({
          ...
          total_price: (spent.quantity_used * (spent.unit_price || 0)).toFixed(
            2
          ),
        }));
  ...
  const filteredStocks = [...spentStocks]
    .sort((a, b) => {
    ...
    .filter(stock => {
      ...
      // Search filter
      const searchString =
        `${stock.item_name} ${stock.project_name} ${stock.employee_name} ${stock.location || ''} ${stock.category_name || ''} ${stock.formatted_date}`.toLowerCase();
                  ...
                  <tbody className="divide-y divide-gray-700">
                    {paginatedStocks.map((spent, index) => (
                      ...
                      >
                        ...
                        <td
                          className="px-6 py-4 max-w-[200px] truncate text-gray-300"
                          title={spent.project_name}
                        >
                          {spent.project_name || 'N/A'}
                        </td>

                        <td className="px-6 py-4 text-gray-300">
                          {spent.employee_name || 'Unknown'}
                        </td>
