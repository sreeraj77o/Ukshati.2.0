...
export default function StockDetails() {
  ...
  const filteredStocks = useMemo(() => {
    return stocks.filter(stock => {
      ...
      const searchString =
        `${stock.item_name} ${stock.category_name}`.toLowerCase();
          ...
          const validatedData = results.data
            .map((row, index) => {
              ...
              const stockItem = stocks.find(
                s =>
                  s.item_name.toLowerCase() ===
                  row['product name'].toLowerCase().trim()
              );

              const projectItem = projects.find(
                p =>
                  p.pname.toLowerCase() ===
                  row['project name'].toLowerCase().trim()
              );

              const employeeItem = employees.find(
                e =>
                  e.name.toLowerCase() ===
                  row['employee name'].toLowerCase().trim()
              );

              if (!stockItem) {
                validationErrors.push(
                  `Row ${index + 2}: Product not found - ${row['product name']}`
                );
                return null;
              }
              ...
              if (!projectItem) {
                validationErrors.push(
                  `Row ${index + 2}: Project not found - ${row['project name']}`
                );
                return null;
              }
              ...
              if (!employeeItem) {
                validationErrors.push(
                  `Row ${index + 2}: Employee not found - ${row['employee name']}`
                );
                return null;
              }
  ...
  const handleBulkSpend = useCallback(async () => {
    ...
    try {
      ...
      for (let i = 0; i < csvData.length; i += batchSize) {
        ...
        setUploadProgress(
          Math.min(100, ((i + batchSize) / csvData.length) * 100)
        );
      }
      ...
      if (failedItems.length > 0) {
        setCsvErrors(
          failedItems.map(
            item => `${item.productName}: ${item.error || 'Unknown error'}`
          )
        );
      } else {
        closeBulkModal();
                  ...
                  <div className="ml-3">
                    ...
                    {notification.quantity !== undefined && (
                      <p className="text-xs mt-1 opacity-80">
                        Current quantity: {notification.quantity}
                      </p>
                    )}
                  </div>
                </motion.div>
                  ...
                  {[
                    ...
                    >
                      {{
                        ...
                        Actions: (
                          <>
                            <FiShoppingCart className="inline-block mr-2 -mt-1 " />{' '}
                            {header}
                          </>
                        ),
                      }[header] || header}
