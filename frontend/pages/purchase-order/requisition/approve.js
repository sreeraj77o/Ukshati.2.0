...
export default function ApproveRequisitions() {
  ...
  const processRequisition = async (id, status) => {
    ...
    try {
      ...
      // Update local state
      setRequisitions(
        requisitions.map(req => (req.id === id ? { ...req, status } : req))
      );
  ...
  return (
    <div className="min-h-screen bg-black text-white p-6">
      ...
      <div className="max-w-6xl mx-auto mt-8">
        ...
        {notification && (
          <div
            className={`mb-6 p-4 ${notification.type === 'success' ? 'bg-green-500/20 border-green-500' : 'bg-red-500/20 border-red-500'} border rounded-lg flex items-center`}
          >
            <FiAlertCircle
              className={
                ...
                >
                  <div className="flex-1">
                    ...
                    <p className="text-gray-400 mt-1">
                      Project: {req.project_name} | Requester:{' '}
                      {req.requester_name}
                    </p>
                    ...
                    {req.status === 'approved' && (
                      <div className="mt-4">
                        <button
                          onClick={() =>
                            router.push(
                              `/purchase-order/orders/new?requisition=${req.id}`
                            )
                          }
