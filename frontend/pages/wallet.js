...
export default function Wallet() {
  ...
  const handleSaveEditModal = async ({ amount }) => {
    ...
    try {
      const response = await fetch('/api/employees/wallet', {
        ...
        body: JSON.stringify({
          employee_id: editModalData.employee.id,
          new_amount_given: parseFloat(amount),
        }),
      });
      if (!response.ok) {
  ...
  useEffect(() => {
    ...
    const fetchEmployees = async () => {
      try {
        const response = await fetch('/api/employees/wallet');
        if (!response.ok)
          throw new Error('Failed to fetch employee wallet data');
        const data = await response.json();
        setEmployees(data);
  ...
  const filteredVisibleEmployees =
  ...
  const handleAddAmountPerProject = async (
    employee_id,
    project_id,
    amountToAdd
  ) => {
  ...
  const downloadCSV = () => {
    ...
    const rows = filteredVisibleEmployees.map(
      ({ name, amount_given, total_expense, balance }) => [
        name,
        amount_given,
        total_expense,
        balance,
        ...
        <div className="w-full max-w-6xl space-y-8">
          ...
          <div className="text-center space-y-4">
            ...
            <p className="text-gray-300 text-lg md:text-xl max-w-2xl mx-auto">
              Track your expenses, view balances, and manage your financial
              overview
            </p>
          </div>
                        ...
                        <React.Fragment key={id}>
                          <tr className="hover:bg-white/10 transition-all text-center font-bold">
                            ...
                            <td className="px-2">
                              ...
                              <button
                                onClick={() =>
                                  handleEditClick({ id, name, amount_given })
                                }
                        ...
                        <div className="bg-gradient-to-br from-gray-900/50 to-gray-800/50 backdrop-blur-xl p-6 rounded-3xl border border-white/10 shadow-2xl transition-all duration-500 hover:shadow-blue-500/20 hover:border-blue-400/30 hover:scale-105">
                          ...
                          <div className="space-y-4 mb-6">
                            ...
                            <div className="flex justify-between items-center p-4 rounded-2xl bg-blue-500/10 border border-blue-500/20">
                              ...
                              <span
                                className={`font-bold text-xl ${balance >= 0 ? 'text-blue-400' : 'text-red-400'}`}
                              >
                                ₹{parseFloat(balance).toFixed(2)}
                              </span>
                            </div>
                          ...
                          {project_expenses.length > 0 && (
                            <div className="mt-6">
                              <h4 className="text-lg font-semibold text-gray-300 mb-3 text-center">
                                Project Breakdown
                              </h4>
                                ...
                                {project_expenses.map(proj => (
                                  ...
                                  >
                                    ...
                                    <span className="text-sm font-semibold text-yellow-400">
                                      ₹
                                      {parseFloat(proj.total_expense).toFixed(
                                        2
                                      )}
                                    </span>
                                  </div>
